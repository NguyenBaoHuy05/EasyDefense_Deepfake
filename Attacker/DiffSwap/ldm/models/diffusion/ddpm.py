    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        for k, v in sd.items():
            if k in model_dict and model_dict[k].shape != v.shape:
                print(f"reshape {k} from {v.shape} to {model_dict[k].shape}")
                sd[k] = sd[k].reshape(model_dict[k].shape)
        ...
        self.print_messages(missing, unexpected)

    @rank_zero_only
    def print_messages(self, missing, unexpected):
        if len(missing) > 0:
            missing = {
                key
                for key in missing
                if "cond_stage_model" not in key and "model_ema" not in key
            }
...
class LatentDiffusion(DDPM):
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ["caption", "coordinates_bbox"]:
                    xc = batch[cond_key]
                elif cond_key == "class_label" or cond_key == "faceattr":
                    xc = batch
                else:
    ...
    @torch.no_grad()
    def sample_log(
        self, cond, batch_size, ddim, ddim_steps, ddim_sampler=None, **kwargs
    ):
        ...
        **kwargs,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log["conditioning"] = xc
            elif self.cond_stage_key == "faceattr":
                log["conditioning"], log["conditioning_lmk"] = log_lmk_and_img(
                    batch["image"], batch["landmark"]
                )
            elif isimage(xc):
                log["conditioning"] = xc
            if ismap(xc):
        ...
        if self.faceswap:
            if self.use_mask:
                mask = torch.sigmoid(self.mask_predictor(z))
            else:
                mask = None
            print("ready to perform faceswap")
            z_start = z[:N]
            for t in [250, 500, 750, 1000]:
                print("sample for t = ", t)
                t -= 1
                tt = repeat(torch.tensor([t]), "1 -> b", b=N)
                tt = tt.to(self.device).long()
                noise = torch.randn_like(z_start)
                z_noisy = self.q_sample(x_start=z_start, t=tt, noise=noise)
                # now sample from z_noisy as x_T
                samples, _ = self.sample_log(
                    cond=c,
                    batch_size=N,
                    ddim=use_ddim,
                    ddim_steps=ddim_steps,
                    eta=ddim_eta,
                    timesteps=int(t / 1000 * 200),
                    x_T=z_noisy,
                )
                x_samples = self.decode_first_stage(samples.to(self.device))
                log[f"faceswap_inter{t}"] = x_samples

                if self.use_mask:
                    z_noisy = z_noisy * mask + z_start * (1 - mask)
                    samples, _ = self.sample_log(
                        cond=c,
                        batch_size=N,
                        ddim=use_ddim,
                    ...
                    samples = samples * mask + z_noisy * (1 - mask)
                    x_samples = self.decode_first_stage(samples.to(self.device))
                    log[f"faceswap_mask_inter{t}"] = x_samples
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(
        ...
        elif self.conditioning_key == "faceshifter":
            # inject multi level attribute
            context = dict(c_crossattn=c_crossattn, c_attr=c_custom)
            out = self.diffusion_model(x, t, context)
