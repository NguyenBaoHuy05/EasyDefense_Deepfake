...
class FaceEmbedder(nn.Module):
    def __init__(
        ...
        self.pair = pair
        cfg = get_config(
            f"src/arcface_torch/configs/{face_dataset}_{face_model}.py"
        )
        self.face_model = get_model(
            cfg.network,
            dropout=0.0,
            fp16=cfg.fp16,
        ...
        a, b = self.face_model.load_state_dict(
        ...
        self.organ_fc = nn.Sequential(
            nn.Linear(3, lmk_dim), nn.GELU(), nn.Linear(lmk_dim, lmk_dim)
        )
    ...
    def extract_organ_feats(self, z, mask_organ):
        if self.merge_eyes:
            ...
            mask_organ_[:, 0] = torch.logical_or(
                mask_organ[:, 0], mask_organ[:, 1]
            )
    ...
    def forward(self, cond, swap=False):
        ...
        if self.organ_attention is not None:
            organ_feats = organ_feats + self.organ_attention(
                self.organ_norm(organ_feats)
            )
        ...
        if self.use_blur:
            return {"c_concat": cond["z_blur"], "c_crossattn": out}
        else:
            return out
