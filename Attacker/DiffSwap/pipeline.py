...
def repair_by_mask(tgt_path='data/portrait/align/target',
                  swap_path='data/portrait/align/source',
                  save_path='data/portrait/swap_res_repair',
                  mask_path='data/portrait/mask/mask'):
    os.makedirs(save_path, exist_ok=True)

    img_list = [f for f in os.listdir(swap_path)
               if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp'))]

    for img_name in tqdm(img_list, desc="Processing images"):

        swap_img_path = os.path.join(swap_path, img_name)
        tgt_img_path = os.path.join(tgt_path, img_name)
        mask_img_path = os.path.join(mask_path, img_name)
        swap_img = cv2.imread(swap_img_path)

        tgt_img = cv2.imread(tgt_img_path)

        mask = matplotlib.image.imread(mask_img_path)
        ...
        gp_swap = gaussian_pyramid(im1)
        gp_target = gaussian_pyramid(im2)

        mask_gp = []
        for level in gp_swap:
            h, w = level.shape[:2]
            resized_mask = cv2.resize(mask, (w, h), interpolation=cv2.INTER_LINEAR)
            resized_mask = np.clip(resized_mask, 0, 1)
            mask_gp.append(resized_mask)


        lp_swap = laplacian_pyramid(gp_swap)
        lp_target = laplacian_pyramid(gp_target)

        lp_combined = laplacian_pyr_join(lp_swap, lp_target, mask_gp)
        result_img = laplacian_collapse(lp_combined)
