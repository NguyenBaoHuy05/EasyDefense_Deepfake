...
def get_image_feature(img_path, files_list, model_path, epoch, gpu_id):
    ...
    for img_index, each_line in enumerate(files[len(files) - rare_size :]):
        ...
        if (img_index + 1) % rare_size == 0:
            print("batch", batch)
            img_feats[len(files) - rare_size :][:] = embedding.forward_db(
                batch_data
            )
            batch += 1
        faceness_scores.append(name_lmk_score[-1])
...
def image2template_feature(img_feats=None, templates=None, medias=None):
    ...
    for count_template, uqt in enumerate(unique_templates):
        ...
        if count_template % 2000 == 0:
            print(
                "Finish Calculating {} template features.".format(
                    count_template
                )
            )
...
def verification(
    template_norm_feats=None, unique_templates=None, p1=None, p2=None
):
    # ==========================================================
...
p1, p2, label = read_template_pair_list(
    os.path.join(
        "%s/meta" % image_path, "%s_template_pair_label.txt" % target.lower()
    )
)
stop = timeit.default_timer()
print("Time: %.2f s. " % (stop - start))
...
print("Time: %.2f s. " % (stop - start))
print(
    "Feature Shape: ({} , {}) .".format(img_feats.shape[0], img_feats.shape[1])
)
...
for method in methods:
    ...
    plt.plot(
        ...
        lw=1,
        label=(
            "[%s (AUC = %0.4f %%)]" % (method.split("-")[-1], roc_auc * 100)
        ),
    )
    tpr_fpr_row = []
