...
from preprocess.mtcnn_pytorch.src.matlab_cp2tform import (
...
NEW_REFERENCE_FACIAL_POINTS = [
    [25.29459953, 51.69630051],
    [60.53179932, 51.50139999],
    [43.02519989, 71.73660278],
    [28.54930115, 92.3655014],
    [57.72990036, 92.20410156],
]
...
def get_reference_facial_points(
    ...
    if (
        ...
        output_size = tmp_crop_size * (1 + inner_padding_factor * 2).astype(
            np.int32
        )
    ...
    if not (
        outer_padding[0] < output_size[0] and outer_padding[1] < output_size[1]
    ):
        raise FaceWarpException(
...
def warp_and_crop_face(
    ...
    if crop_size[0] == 96 and crop_size[1] == 112:
        ref_pts[:, 0] = (ref_pts[:, 0] - crop_size[0] / 2) * 0.85 + crop_size[
            0
        ] / 2
        ref_pts[:, 1] = (ref_pts[:, 1] - crop_size[1] / 2) * 0.85 + crop_size[
            1
        ] / 2
    else:
    ...
    if return_trans_inv and align_type is "smilarity":
