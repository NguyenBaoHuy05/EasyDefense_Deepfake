...
class fsModel(BaseModel):
    ...
    def init_loss_filter(self, use_gan_feat_loss, use_vgg_loss):
        ...
        def loss_filter(
            g_gan, g_gan_feat, g_vgg, g_id, g_rec, g_mask, d_real, d_fake
        ):
            return [
                l
                for (l, f) in zip(
    ...
    def initialize(self, opt):
        ...
        if (
            opt.resize_or_crop != "none" or not opt.isTrain
        ...
        if opt.continue_train or opt.load_pretrain:
            ...
            self.load_network(self.netG, "G", opt.which_epoch, pretrained_path)
            self.load_network(
                self.netD1, "D1", opt.which_epoch, pretrained_path
            )
            self.load_network(
                self.netD2, "D2", opt.which_epoch, pretrained_path
            )
        ...
        if self.isTrain:
            ...
            self.criterionGAN = networks.GANLoss(
                opt.gan_mode, tensor=self.Tensor, opt=self.opt
            )
            ...
            # optimizer D
            params = list(self.netD1.parameters()) + list(
    ...
    def forward(self, img_id, img_att, latent_id, latent_att, for_G=False):
        ...
        loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_G_ID, loss_G_Rec = (
            0,
            0,
            0,
            0,
            ...
            for i in range(num_D):
                for j in range(0, len(fea_fake[i]) - 1):
                    loss_G_GAN_Feat += (
                        ...
                        * feat_weights
                        * self.criterionFeat(
                            fea_fake[i][j], fea_real[i][j].detach()
                        )
