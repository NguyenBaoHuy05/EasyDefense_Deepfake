...
class ImageEditor:
    def __init__(self, args) -> None:
        ...
        self.ranked_results_path = Path(
            os.path.join(self.args.output_path, RANKED_RESULTS_DIR)
        )
    ...
    def edit_image_by_prompt(self):
            ...
            with torch.enable_grad():
                ...
                if t < 50 and t > 10:
                    ...
                    if llx is not None:
                        ...
                        src_left_eye = torch.mean(
                            src_left_eye, dim=1, keepdim=True
                        )
                        ...
                        targ_right_gaze = self.netGaze(
                            targ_right_eye.squeeze(0)
                        )
                        src_right_gaze = self.netGaze(src_right_eye.squeeze(0))
                        left_gaze_loss = l1_loss(targ_left_gaze, src_left_gaze)
                        right_gaze_loss = l1_loss(
                            targ_right_gaze, src_right_gaze
                        )
                ...
                self.metrics_accumulator.update_metric(
                    "l2_loss",
                    mse_loss(masked_background, self.targ_image).item(),
                )
                self.metrics_accumulator.update_metric(
                    "bg_loss",
                    mse_loss(
                        masked_background, self.targ_image * (1 - self.mask)
                    ).item(),
                )
                # ------------------------------------------------------------------------------------------------------------------------ #
        ...
        def postprocess_fn(out, t):
            ...
            if self.mask is not None:
                background_stage_t = self.diffusion.q_sample(
                    self.targ_image, t[0]
                )
                ...
                softmask = self.mask * (
                    min(
                        1,
                        (75 - (t.data + 1))
                        / (75.0 - self.args.masking_threshold),
                    )
                )
                ...
                if self.args.enforce_background:
                    out["sample"] = out[
                        "sample"
                    ] * softmask + background_stage_t * (1 - softmask)
                else:
        ...
        src_loader = DataLoader(
            src_dataset, num_workers=4, shuffle=False, batch_size=1
        )
        targ_loader = DataLoader(
            targ_dataset, num_workers=4, shuffle=False, batch_size=1
        )
        ...
        for step in range(length):
            ...
            for iteration_number in range(
                ...
                total_steps = (
                    self.diffusion.num_timesteps - self.args.skip_timesteps - 1
                )
                for j, sample in enumerate(samples):
                    should_save_image = (
                        j % save_image_interval == 0 or j == total_steps
                    )
                    ...
                    if should_save_image:
                        ...
                        pred_src_mask = transforms.Resize((512, 512))(
                            pred_src_mask.unsqueeze(0)
                        )
                        pred_src_mask = transforms.Normalize(
                            (0.485, 0.456, 0.406), (0.229, 0.224, 0.225)
                        )(pred_src_mask)
                        pred_src_mask = self.netSeg(self.spNorm(pred_src_mask))[
                            0
                        ]
                        pred_src_mask = transforms.Resize((256, 256))(
                            pred_src_mask
                        )
                        parsing = (
                            pred_src_mask.squeeze(0)
                            .detach()
                            .cpu()
                            .numpy()
                            .argmax(0)
                        ...
                        _src_base = torch.from_numpy(src_base)[:, :, 0].to(
                            self.device
                        )
                        _src_base = _src_base.unsqueeze(0).unsqueeze(0)

                        if self.mask is not None and j == total_steps:
                            if self.args.enforce_background:
                                pred_image = (
                                    self.targ_image[0] * (1 - self.mask[0])
                                    + pred_image * self.mask[0]
                                )
                            else:
                                pred_image = (
                                    self.targ_image[0] * (1 - _src_base[0])
                                    + pred_image * _src_base[0]
                                )
                        ...
                        src_image_pil = (
                            self.src_image[0].add(1).div(2).clamp(0, 1)
                        )
                        src_image_pil = TF.to_pil_image(src_image_pil)

                        targ_image_pil = (
                            self.targ_image[0].add(1).div(2).clamp(0, 1)
                        )
                        ...
                        final_distance = self.id_distance(
                            pred_image_pil, src_image_pil
                        )
