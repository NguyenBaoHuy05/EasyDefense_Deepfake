...
def id_distance(src, targ, netArc, device):
    ...
    src = transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])(
        src
    )
    ...
    targ = transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])(
        targ
    )
...
if __name__ == "__main__":
    ...
    src_loader = DataLoader(
        src_dataset, num_workers=1, shuffle=False, batch_size=1
    )
    targ_loader = DataLoader(
        targ_dataset, num_workers=1, shuffle=False, batch_size=1
    )
    ...
    for step in range(length):
        ...
        targ_img = (
            Image.open(targ_instance_path[0])
            .convert("RGB")
            .resize((args.resolution, args.resolution))
        )
        src_img = (
            Image.open(instance_path[0])
            .convert("RGB")
            .resize((args.resolution, args.resolution))
        )
        ...
        lora_path = os.path.join(
            args.output_dir, f"lora/{src_img_name}_lora_weight.safetensors"
        )
        patch_pipe(
            pipe,
            lora_path,
        ...
        image = pipe(
            ...
            src_image=src_img,
            lora_path=os.path.join(
                args.output_dir, f"lora/{src_img_name}_lora_weight.pt"
            ),
            args_seed=args.seed,
        ).images[0]
        # swap face results
