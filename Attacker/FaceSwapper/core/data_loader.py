...
class TrainFaceDataSet(data.Dataset):
    def __init__(self, data_path_list, transform=None, transform_seg=None):
        ...
        self.num_per_folder = []
        self.lm_image_path = (
            data_path_list[0][: data_path_list[0].rfind("/") + 1]
            + data_path_list[0][data_path_list[0].rfind("/") + 1 :]
            + "_lm_images/"
        )
    ...
    def __getitem__(self, item):
        ...
        reference_lm_image_path = (
            self.lm_image_path + reference_image_path.split("/")[-1]
        )
        reference_mask_image_path = (
            self.mask_image_path + reference_image_path.split("/")[-1]
        )
        reference_image = Image.open(reference_image_path).convert("RGB")
        reference_lm_image = Image.open(reference_lm_image_path).convert("RGB")
        reference_mask_image = Image.open(reference_mask_image_path).convert(
            "L"
        )
        if self.transform is not None:
...
class TestFaceDataSet(data.Dataset):
    def __init__(
        self, data_path_list, test_img_list, transform=None, transform_seg=None
    ):
        self.source_dataset = []
        self.reference_dataset = []
        self.data_path_list = data_path_list
        self.lm_image_path = (
            data_path_list[: data_path_list.rfind("/") + 1]
            + data_path_list[data_path_list.rfind("/") + 1 :]
            + "_lm_images/"
        )
        self.mask_image_path = (
    ...
    def __getitem__(self, item):
        source_image_path = (
            self.data_path_list + "/" + self.source_dataset[item]
        )
        try:
        ...
        source_parsing_image_path = (
            self.biseg_parsing_path + self.source_dataset[item]
        )
        source_lm_image = Image.open(souce_lm_image_path).convert("RGB")
        source_mask_image = Image.open(souce_mask_image_path).convert("L")
        source_parsing_image = Image.open(source_parsing_image_path).convert(
            "L"
        )
        if self.transform is not None:
            source_image = self.transform(source_image)
        ...
        reference_image_path = (
            self.data_path_list + "/" + self.reference_dataset[item][0:-1]
        )
        try:
            reference_image = Image.open(reference_image_path).convert("RGB")
        except:
        ...
        reference_lm_image_path = (
            self.lm_image_path + self.reference_dataset[item][0:-1]
        )
        reference_mask_image_path = (
            self.mask_image_path + self.reference_dataset[item][0:-1]
        )
        reference_parsing_image_path = (
            self.biseg_parsing_path + self.reference_dataset[item][0:-1]
        )
        reference_lm_image = Image.open(reference_lm_image_path).convert("RGB")
        reference_mask_image = Image.open(reference_mask_image_path).convert(
            "L"
        )
        reference_parsing = Image.open(reference_parsing_image_path).convert(
            "L"
        )
        if self.transform is not None:
            reference_image = self.transform(reference_image)
        ...
        outputs = dict(
            ...
            src_mask=1 - source_mask_image,
            ref_mask=1 - reference_mask_image,
            src_parsing=source_parsing,
            ref_parsing=reference_parsing,
...
def get_train_loader(root, img_size=256, batch_size=8, num_workers=4):
...
def get_test_loader(
    root, test_img_list, img_size=256, batch_size=8, num_workers=4
):
    ...
    test_dataset = TestFaceDataSet(
        root, test_img_list, transform, transform_seg
    )
...
class InputFetcher:
    ...
    def _fetch_inputs(self):
        inputs_data = next(iter(self.loader))
        return inputs_data
    ...
    def __next__(self):
        ...
        inputs = Munch(
            ...
            src_mask=t_inputs["src_mask"],
            tar_mask=t_inputs["ref_mask"],
        )
        if self.mode == "train":
            inputs = Munch({k: t.to(self.device) for k, t in inputs.items()})
        elif self.mode == "test":
            inputs = Munch(
                {k: t.to(self.device) for k, t in inputs.items()},
                src_parsing=t_inputs["src_parsing"].to(self.device),
                tar_parsing=t_inputs["ref_parsing"].to(self.device),
                src_name=t_inputs["src_name"],
                tar_name=t_inputs["ref_name"],
            )
        return inputs
