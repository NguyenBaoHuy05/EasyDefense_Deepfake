...
def infer(checkpoint_path, ckpt_pipeline, prompts=None, n_img=16, bs=8, n_steps=100, guidance_scale=7.5):
    if ckpt_pipeline is None:
        pipe = StableDiffusionPipeline.from_pretrained(
            checkpoint_path, torch_dtype=torch.bfloat16, safety_checker=None
        ).to("cuda")
    else:   
        pipe = ckpt_pipeline.to("cuda")
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        if global_step < 1000:
                            ...
                            infer(save_path, ckpt_pipeline, prompts, n_img=16, bs=4, n_steps=100)
                        else:
