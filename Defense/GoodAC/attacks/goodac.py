...
from canny_loss import batch_process
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="local face attribute distortion guidance prompt",
    )
    parser.add_argument(
        "--class_prompt",
        type=str,
...
def pgd_attack(
    ...
    for step in range(num_steps):
        ...
        noise_out_layers_features_0, noise_out_layers_features_1, noise_out_layers_features_2, noise_out_layers_features_3, \
            noise_in_layers_features_0, noise_in_layers_features_1, noise_in_layers_features_2, noise_in_layers_features_3 = save_feature_maps(
            unet.up_blocks, unet.down_blocks)
        with torch.no_grad():
            ...
            clean_out_layers_features_0, clean_out_layers_features_1, clean_out_layers_features_2, clean_out_layers_features_3, \
                clean_in_layers_features_0, clean_in_layers_features_1, clean_in_layers_features_2, clean_in_layers_features_3 = save_feature_maps(
                unet.up_blocks, unet.down_blocks)
        noise_out_layers_features_3_mix = shuffle_rotate_mix_channels(noise_out_layers_features_3, 47, 20, True)
        target_loss_mix_3 = F.mse_loss(noise_out_layers_features_3_mix.float(), clean_out_layers_features_3.float(),
                                     reduction="mean")
        noise_in_layers_features_0_mix = shuffle_rotate_mix_channels(noise_in_layers_features_0, 47, 20, True)
        target_loss_mix_1 = F.mse_loss(noise_in_layers_features_0_mix.float(), clean_in_layers_features_0.float(),
                                     reduction="mean")
        canny_loss = batch_process(noise_out_layers_features_3, clean_out_layers_features_3)
        target_loss = F.mse_loss(noise_out_layers_features_3.float(), clean_out_layers_features_3.float(),
                                 reduction="mean")
        unet.zero_grad()
        text_encoder.zero_grad()
        loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
        loss = loss + target_loss_mix_1.detach().item() + target_loss_mix_3.detach().item() + args.beta * canny_loss + args.gamma * target_loss.detach().item()
        loss.backward()
        ...
        print(f"PGD loss - step {step}, loss: {loss.detach().item()}, target_loss_mix_1 : {target_loss_mix_1.detach().item()}, target_loss_mix_3 : {target_loss_mix_3.detach().item()}")
    return perturbed_images


def shuffle_rotate_mix_channels(x, num_blocks, max_angle, mix_channels=False):
    if len(x.shape) != 5:
        raise ValueError("Input tensor must be 5-dimensional (batch, sub_batch, channels, height, width).")
    batch_size, sub_batch_size, channels, height, width = x.shape
    print("batch_size, sub_batch_size, channels, height, width", batch_size, sub_batch_size, channels, height, width)
    original_dtype = x.dtype
    if x.dtype == torch.bfloat16:
        x = x.float()
    # Helper function to calculate split sizes for each dimension
    def get_length(length, num_blocks):
        if length < num_blocks:
            raise ValueError("Block number must be less than or equal to the dimension size.")
        basic_block_size = length // num_blocks
        remainder = length % num_blocks
        lengths = [basic_block_size + 1 if i < remainder else basic_block_size for i in range(num_blocks)]
        return tuple(lengths)
    height_splits = get_length(height, num_blocks)
    width_splits = get_length(width, num_blocks)
    # Check for zero-sized splits and adjust if necessary
    if any(h == 0 for h in height_splits) or any(w == 0 for w in width_splits):
        raise ValueError("Zero-sized split detected, adjust the number of blocks or image dimensions.")
    # Prepare the output tensor
    x_rotated = torch.empty_like(x)
    # Process each feature map independently
    for b in range(batch_size):
        for s in range(sub_batch_size):
            current_img = x[b, s]
            # Randomly permute the sections of the image
            height_perm = torch.randperm(num_blocks)
            width_perm = torch.randperm(num_blocks)
            # Split and permute blocks along width
            x_split_w = torch.split(current_img, width_splits, dim=2)
            x_w_perm = torch.cat([x_split_w[i] for i in width_perm], dim=2)
            # Generate random rotation angles for the feature map
            angles = torch.normal(0, max_angle * 0.05, size=(1,))
            # Split, permute and rotate blocks along height
            strip_rotated = []
            for strip_index in range(num_blocks):
                strip = x_w_perm[:, :,
                        strip_index * height_splits[strip_index]:strip_index * height_splits[strip_index] +
                                                                 height_splits[strip_index]]
                strip_split_h = torch.split(strip, height_splits, dim=1)
                strip_rotated.append(torch.cat([
                    TF.rotate(strip_split_h[i], angle=float(angles[0]))
                    for i in height_perm
                ], dim=1))
            # Concatenate rotated blocks back into full images
            current_img_rotated = torch.cat(strip_rotated, dim=2)
            # Mix channels if required
            if mix_channels:
                current_img_rotated = current_img_rotated[torch.randperm(channels), :, :]
            # Store the processed feature map back into the output tensor
            x_rotated[b, s] = current_img_rotated
    # Convert tensor back to original dtype if necessary
    if original_dtype == torch.bfloat16:
        x_rotated = x_rotated.to(dtype=torch.bfloat16)
    return x_rotated
                ...
                for index in range(0, 5):
                    ...
                    if index == 1:
                        ...
                        if max_score < score:
                            ...
                            select_t = res_time_seq[inner_index].item()
                    print(
                        f"PGD loss - step {step}, index : {index}, loss: {loss.detach().item()}, score: {score}, t : {res_time_seq[inner_index]}, ts_len: {len(res_time_seq)}")
...
def main(args):
    ...
    if args.target_image_path is not None:
        ...
        target_latent_tensor = (
                vae.encode(target_image_tensor).latent_dist.sample().to(
                    dtype=torch.bfloat16) * vae.config.scaling_factor
        )
